...
// MaskedClientErrorMessage 面向客户的通用错误文案，不含任何上游/内部细节，
// 仅附带一个供客服排查的 request id。开启 MaskRelayClientError 时统一使用。
func MaskedClientErrorMessage(requestId string) string {
	if requestId == "" {
		return "服务暂时不可用，请稍后重试；如持续失败请联系客服。"
	}
	return fmt.Sprintf("服务暂时不可用，请稍后重试；如持续失败请联系客服并提供编号：%s", requestId)
}

// InsufficientQuotaMessage 余额/额度不足提示。客户充值即可解决,明确告知避免误判为系统故障。
func InsufficientQuotaMessage() string {
	return "账户额度不足，请充值后重试（当前余额已不足以支付本次请求）。"
}

// ClientRequestHint 针对"客户请求类"错误(400/413/422/429)的脱敏但可操作提示。
// 保留原始状态码让客户端 agent 正确处理(修请求/退避)，避免收到无信息 500 后盲目重试反而烧更多 token。
// 不泄露任何上游品牌/细节。
func ClientRequestHint(statusCode int, requestId string) string {
	var base string
	switch statusCode {
	case 413:
		base = "请求过大或上下文过长，请缩短后重试"
	case 429:
		base = "请求过于频繁，请降低并发或速率后重试"
	default: // 400 / 422 等
		base = "请求无效（可能参数错误、内容不被接受或上下文过长），请检查请求后重试"
	}
	if requestId == "" {
		return base + "。"
	}
	return fmt.Sprintf("%s；如需协助请联系客服并提供编号：%s", base, requestId)
}
