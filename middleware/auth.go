...
func authHelper(c *gin.Context, minRole int) {
	...
	if username == nil {
		...
		if user != nil && user.Username != "" {
			...
			// IP 白名单：用户配了 AccessTokenAllowIps 就强校验
			allowIps := user.GetAccessTokenIpLimits()
			if len(allowIps) > 0 {
				clientIp := c.ClientIP()
				ip := net.ParseIP(clientIp)
				if ip == nil || !common.IsIpInCIDRList(ip, allowIps) {
					common.SysLog(fmt.Sprintf("access_token blocked: user=%d clientIP=%s not in whitelist", user.Id, clientIp))
					c.JSON(http.StatusForbidden, gin.H{
						"success": false,
						"message": "您的 IP (" + clientIp + ") 不在系统令牌允许列表中",
					})
					c.Abort()
					return
				}
