...
// 订阅升级分组 key → 友好通道名，镜像前端 web/default/src/features/subscriptions/lib/format.ts。
// 仅管理员可见的操作反馈消息使用；未映射的 key(default / 历史组)原样返回，避免把回退目标误标。
var subscriptionGroupLabels = map[string]string{
	"sub-trial":    "体验通道",
...
func friendlySubscriptionGroup(group string) string {
	g := strings.TrimSpace(group)
	if g == "" {
		return ""
	}
	if name, ok := subscriptionGroupLabels[g]; ok {
		return name
	}
	return g
}
...
func AdminBindSubscription(userId int, planId int, sourceNote string) (string, error) {
	...
	if strings.TrimSpace(plan.UpgradeGroup) != "" {
		...
		return fmt.Sprintf("用户分组将升级到 %s", friendlySubscriptionGroup(plan.UpgradeGroup)), nil
	}
	return "", nil
}
...
func AdminInvalidateUserSubscription(userSubscriptionId int) (string, error) {
	...
	if downgradeGroup != "" {
		return fmt.Sprintf("用户分组将回退到 %s", friendlySubscriptionGroup(downgradeGroup)), nil
	}
	return "", nil
}
