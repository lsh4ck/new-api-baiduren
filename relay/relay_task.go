...
func videoFetchByIDRespBodyBuilder(c *gin.Context) (respBody []byte, taskResp *dto.TaskError) {
	...
	// 通用 TaskDto 格式（TaskModel2Dto 内已对 Atlas 视频/图像脱敏）
...
// maskVideoResultURL 隐藏上游视频直链：把对客响应里的真实上游 URL 换成本站代理 URL
// (/v1/videos/{id}/content)。真实 URL 仍存于 task.PrivateData，由 controller.VideoProxy
// 取用并流式转发，客户全程只看到本站域名，不泄露上游存储域名/品牌。
// 仅处理绝对 http(s) 直链；data: URI 与已是代理路径的不动。
func maskVideoResultURL(task *model.Task, d *dto.TaskDto) {
	if d == nil {
		return
	}
	// 仅对 Atlas 视频(59)/图像(60) 任务脱敏，避免影响 MJ/Suno 等其它 task 类型
	if string(task.Platform) != strconv.Itoa(constant.ChannelTypeAtlas) &&
		string(task.Platform) != strconv.Itoa(constant.ChannelTypeAtlasImage) {
		return
	}
	...
	d.ResultURL = taskcommon.BuildProxyURL(task.TaskID)
	// 原始上游响应体(含直链)不下发给客户；状态已由 TaskDto 各字段体现
	d.Data = nil
}
...
func TaskModel2Dto(task *model.Task) *dto.TaskDto {
	...
	// Atlas 视频/图像统一脱敏：result_url 换代理 url、丢弃含上游直链的 Data
	// （fetch 单条 + /api/task/self 列表 都经此函数，保证作品库不泄露上游品牌）
	maskVideoResultURL(task, d)
	return d
}
