...
func OpenaiHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// 上游返回 200 但无 choices（号池/中转把错误塞进 200 body，绕过状态预检）。
	// 不能裸拷/转成空响应给客户，否则会透传上游欠费/品牌。识别欠费→503,其余→坏响应可级联重试。
	if len(simpleResponse.Choices) == 0 {
		raw := string(responseBody)
		if types.IsUpstreamBalanceErrorText(raw) {
			return nil, types.NewErrorWithStatusCode(fmt.Errorf("upstream insufficient balance: %s", raw), types.ErrorCodeUpstreamNoBalance, http.StatusServiceUnavailable)
		}
		return nil, types.NewErrorWithStatusCode(fmt.Errorf("invalid upstream response (no choices): %s", raw), types.ErrorCodeBadResponse, http.StatusBadGateway)
	}
