...
/**
 * Update access-token IP whitelist (newline / comma separated IP or CIDR; empty string clears).
 */
export async function updateAccessTokenAllowIps(
  allowIps: string
): Promise<ApiResponse<string>> {
  const res = await api.put('/api/user/token/allow_ips', {
    allow_ips: allowIps,
  })
  return res.data
}
