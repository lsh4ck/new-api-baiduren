...
// First-time profile setup for users created via OAuth without email/password
export async function initialProfileSetup(payload: {
  username: string
  email: string
  verification_code: string
  password: string
}): Promise<ApiResponse> {
  const res = await api.post('/api/user/self/initial-setup', payload)
  return res.data
}
