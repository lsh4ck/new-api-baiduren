...
export function loadMessages(): Message[] | null {
  try {
    ...
    if (saved) {
      ...
      // 过滤 corrupt message（versions 字段缺失/null），防止渲染时 throw
      const safe = (parsed || []).filter(
        (m) => m && typeof m === 'object' && Array.isArray(m.versions) && m.versions.length > 0
      )
      const sanitized = sanitizeMessagesOnLoad(safe)
      // Persist sanitized result to avoid re-sanitizing on subsequent loads
      if (sanitized.length !== parsed.length || sanitized !== safe) {
    ...
    // 解析失败 → 清空避免反复触发
    try { localStorage.removeItem(STORAGE_KEYS.MESSAGES) } catch {}
  }
  return null
}
