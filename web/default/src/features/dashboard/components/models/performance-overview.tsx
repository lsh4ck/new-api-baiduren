...
function weightedAverage(
  ...
  for (const row of rows) {
    ...
    if (requestCount <= 0 || requestCount < minSamples || !isValid(value)) continue
            ...
            <Table className='text-sm'>
              ...
              <TableBody>
                {loading
                  ...
                  : topModels.map((model) => {
                      const lowSample = model.request_count < MIN_SAMPLE_FOR_RATE
                      return (
                      <TableRow key={model.model_name}>
                        ...
                        <TableCell
                          ...
                          title={
                            lowSample
                              ? `样本仅 ${model.request_count} 次，统计意义不足`
                              : undefined
                          }
                        >
                          <span className='inline-flex items-center justify-end gap-1.5'>
                            ...
                            {lowSample ? (
                              <span className='text-xs font-normal italic'>
                                样本不足 ({formatUptimePct(model.success_rate)})
                              </span>
                            ) : (
