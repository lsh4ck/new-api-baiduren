...
function BillingBreakdown(props: {
  ...
  const rawBaseInputUSD = other.model_ratio != null ? other.model_ratio * 2.0 : 0

  // 优先用 user_group_ratio（专属客户独立倍率），否则用 group_ratio
  const _userGRForPrice = other.user_group_ratio
  const _isUserGRForPrice =
    _userGRForPrice != null &&
    Number.isFinite(_userGRForPrice) &&
    _userGRForPrice !== -1
  const _effectiveGRForPrice = _isUserGRForPrice
    ? _userGRForPrice
    : other.group_ratio
  const _grMultiplier =
    _effectiveGRForPrice != null && Number.isFinite(_effectiveGRForPrice)
      ? _effectiveGRForPrice
      : 1
  // baseInputUSD 已含 GR 折扣，所有派生显示价（input/output/cache/audio/image）自动含折扣
  const baseInputUSD = rawBaseInputUSD * _grMultiplier
