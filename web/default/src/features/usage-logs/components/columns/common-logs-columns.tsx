...
function buildDetailSegments(
  ...
  if (log.type === 5) {
    // 错误日志：展示状态码 + 错误内容
    const segs: DetailSegment[] = [{ text: '❌ ' + t('Error'), danger: true }]
    const content = log.content || ''
    // 提取 status_code（content 开头一般是 "status_code=XXX, ..."）
    const m = content.match(/status_code=(\d+)/)
    if (m) {
      segs.push({ text: 'HTTP ' + m[1], muted: true })
    }
    // 截取 content 前 200 字符避免太长
    const msg = content.replace(/^status_code=\d+,?\s*/, '').slice(0, 200)
    if (msg) segs.push({ text: msg })
    if (other && typeof other === 'object') {
      const ot = other as Record<string, unknown>
      if (ot.error_type) segs.push({ text: 'type=' + String(ot.error_type), muted: true })
      if (ot.channel_name) segs.push({ text: 'channel=' + String(ot.channel_name), muted: true })
    }
    return segs
  }
