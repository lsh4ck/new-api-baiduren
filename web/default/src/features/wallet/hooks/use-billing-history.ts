...
export function useBillingHistory(options: UseBillingHistoryOptions = {}) {
  ...
  /**
   * Delete own pending/failed/expired order
   */
  const handleDeleteOrder = useCallback(
    async (id: number) => {
      setDeleting(true)
      try {
        const response = await deleteSelfTopUp(id)
        if (isApiSuccess(response)) {
          toast.success(i18next.t('Order deleted'))
          await fetchBillingHistory()
          return true
        }
        toast.error(response.message || i18next.t('Failed to delete order'))
        return false
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error('Failed to delete order:', error)
        toast.error(i18next.t('Failed to delete order'))
        return false
      } finally {
        setDeleting(false)
      }
