...
/**
 * Delete user's own pending/failed/expired topup order
 */
export async function deleteSelfTopUp(id: number): Promise<ApiResponse> {
  const res = await api.delete(`/api/user/topup/self/${id}`)
  return res.data
}
