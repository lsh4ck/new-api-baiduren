...
function SidebarMenuLink({ item, href }: { item: NavLink; href: string }) {
  ...
  const isMarquee = item.highlight === 'marquee'
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        isActive={checkIsActive(href, item)}
        tooltip={item.title}
        className={isMarquee ? 'nav-marquee relative overflow-hidden font-semibold' : undefined}
        render={
          item.external ? (
            <a
              href={item.url}
      ...
      >
        {item.icon && <item.icon />}
        <span className={isMarquee ? 'relative z-10' : undefined}>{item.title}</span>
        {item.badge && <NavBadge>{item.badge}</NavBadge>}
        {isMarquee && (
          <span className='ms-auto relative z-10 inline-flex size-1.5 animate-pulse rounded-full bg-emerald-500' />
        )}
      </SidebarMenuButton>
    </SidebarMenuItem>
  )
