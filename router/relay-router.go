...
func SetRelayRouter(router *gin.Engine) {
	...
	// 漫剧成片渲染（Bearer token 鉴权，发往独立渲染机；不走 Distribute，无上游渠道）
	renderRouter := router.Group("/api/render")
	renderRouter.Use(middleware.TokenAuth())
	{
		renderRouter.POST("", controller.RenderSubmit)
		renderRouter.GET("/:id", controller.RenderStatus)
		renderRouter.GET("/:id/content", controller.RenderContent)
	}
	...
	// ===== 订阅中继路由 (使用 TokenAuth 认证) =====
	subscriptionRelayRouter := router.Group("/v1/subscription")
	subscriptionRelayRouter.Use(middleware.RouteTag("subscription"))
	subscriptionRelayRouter.Use(middleware.TokenAuth())
	subscriptionRelayRouter.Use(middleware.SubscriptionRateLimit())
	{
		subscriptionRelayRouter.POST("/claude/messages", relay.RelayClaude)
		subscriptionRelayRouter.POST("/codex/completions", relay.RelayCodex)
		subscriptionRelayRouter.POST("/gemini/generateContent", relay.RelayGemini)
		subscriptionRelayRouter.POST("/bind", relay.BindStickyAccount)
		subscriptionRelayRouter.POST("/reset", relay.ResetStickyAccount)
		subscriptionRelayRouter.GET("/status", relay.GetSubscriptionAccountStatus)
		subscriptionRelayRouter.GET("/health", relay.SubscriptionHealthCheck)
	}
}
