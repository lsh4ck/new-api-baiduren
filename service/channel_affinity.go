...
// InvalidateChannelAffinityForRequest 清除当前请求关联的亲和性缓存条目。
// 用途：当亲和性命中的渠道被禁用时，删除这条已失效的绑定，避免下次同请求继续撞死。
func InvalidateChannelAffinityForRequest(c *gin.Context) {
	if c == nil {
		return
	}
	...
	if _, err := cache.DeleteMany([]string{key}); err != nil {
		common.SysError(fmt.Sprintf("invalidate channel affinity for request failed: key=%s err=%v", key, err))
	}
}
