...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	// 累加企业/工作组/个人限额计数，并触发软告警
	if quota > 0 && relayInfo != nil && relayInfo.UserId > 0 {
		if err := model.AccumulateLimitsForUser(relayInfo.UserId, int64(quota)); err != nil {
			common.SysError("failed to accumulate enterprise limits: " + err.Error())
		} else {
			uid := relayInfo.UserId
			gopool.Go(func() { TriggerSoftAlertsForUser(uid) })
		}
	}
