...
func PreConsumeQuota(c *gin.Context, preConsumedQuota int, relayInfo *relaycommon.RelayInfo) *types.NewAPIError {
	// 检查企业级硬限额（员工/工作组/企业三层）
	if blocker, _ := model.CheckHardLimitsForUser(relayInfo.UserId, int64(preConsumedQuota)); blocker != nil {
		scope := map[string]string{
			model.LimitScopeEnterprise: "企业总额",
			model.LimitScopeWorkGroup:  "工作组",
			model.LimitScopeMember:     "个人",
		}[blocker.ScopeType]
		period := map[string]string{
			model.LimitPeriodDaily:   "日",
			model.LimitPeriodMonthly: "月",
			model.LimitPeriodQuarter: "季度",
			model.LimitPeriodTotal:   "总",
		}[blocker.Period]
		msg := fmt.Sprintf("已达到%s%s额度上限(%s/%s)，请联系企业管理员",
			scope, period,
			logger.FormatQuota(int(blocker.UsedQuota)),
			logger.FormatQuota(int(blocker.MaxQuota)),
		)
