...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	appendSmartRelayInfo(ctx, other)
	return other
}

// appendSmartRelayInfo 把 captureSmartRelayHeaders 暂存的 SmartRelay 节省指标写入 log.Other，
// 后续用量详情/优化报告就能从 logs.other 解析出每条请求的节省量与缓存命中情况。
func appendSmartRelayInfo(ctx *gin.Context, other map[string]interface{}) {
	if ctx == nil || other == nil {
		return
	}
	if v, exists := ctx.Get("smartrelay_tokens_saved"); exists {
		if n, ok := v.(int); ok && n > 0 {
			other["sr_saved"] = n
		}
	}
	if v, exists := ctx.Get("smartrelay_save_type"); exists {
		if s, ok := v.(string); ok && s != "" {
			other["sr_type"] = s
		}
	}
	if v, exists := ctx.Get("smartrelay_cache_hit"); exists {
		if b, ok := v.(bool); ok && b {
			other["sr_cache_hit"] = true
		}
	}
}
