...
// groupCacheRatioMap: 按分组覆盖缓存读倍率(group→ratio)。用于给特定客户组(如 luouy特价组)
// 设置与 OR/官网一致的缓存读 0.1×,实现精确折扣对齐。命中时优先于按模型的 cacheRatioMap。
var groupCacheRatioMap = types.NewRWMap[string, float64]()
...
// GroupCacheRatio2JSONString converts the group cache ratio map to a JSON string
func GroupCacheRatio2JSONString() string {
	return groupCacheRatioMap.MarshalJSONString()
}

// UpdateGroupCacheRatioByJSONString updates the group cache ratio map from a JSON string
...
// GetCacheRatioWithGroup 优先使用分组覆盖的缓存读倍率,否则回退到按模型的缓存读倍率。
func GetCacheRatioWithGroup(name string, group string) (float64, bool) {
	if group != "" {
		if ratio, ok := groupCacheRatioMap.Get(group); ok {
			return ratio, true
		}
	}
