...
func testChannel(channel *model.Channel, testModel string, endpointType string, isStream bool) testResult {
	...
	var unsupportedTestChannelTypes = []int{
		...
		constant.ChannelTypeAtlas,
		constant.ChannelTypeAtlasImage,
	}
...
// isImageGenerationTestModel 判断模型名是否会触发图片生成（用作自动测试跳过依据）
// 图片渠道每次测试都消耗真实 GPU 算力，gpt-image-2/dall-e/imagen 系列 1024×1024 一次 ≈ $0.06+
func isImageGenerationTestModel(name string) bool {
	if name == "" {
		return false
	}
	lower := strings.ToLower(name)
	imageTriggers := []string{
		"image", "imagen", "dall-e", "dalle", "stable-diffusion", "sd-",
		"flux", "midjourney", "nano-banana", "ideogram", "grok-imagine",
		"kling", "luma", "runway", "wan2",
		// 视频生成与图像同样昂贵(测一次=真出片),一并跳过自动测试。
		// 否则 #144 Atlas视频(test_model 空→默认首个模型 seedance-2.0/text-to-video)
		// 会每轮真生成一条 1080p 视频烧上游余额(~$1/次),是 #144 被烧干的元凶之一。
		"seedance", "veo", "vidu", "hailuo", "sora", "wan-2", "wan2.",
		"text-to-video", "image-to-video", "reference-to-video",
	}
	for _, kw := range imageTriggers {
...
func testAllChannels(notify bool) error {
	...
	gopool.Go(func() {
		...
		for _, channel := range channels {
			...
			// 跳过自动测试图片生成渠道：测一次 1024×1024 真图 ≈ ¥4.37，每 3h 跑全量会烧穿管理员额度（admin buyaogaowo 30 天烧 ¥366）
			// 手动单测仍可通过 UI 触发；批量自动测试只测文本/embedding 类
			autoTestModel := ""
			if channel.TestModel != nil && *channel.TestModel != "" {
				autoTestModel = strings.TrimSpace(*channel.TestModel)
			} else if ms := channel.GetModels(); len(ms) > 0 {
				autoTestModel = strings.TrimSpace(ms[0])
			}
			if isImageGenerationTestModel(autoTestModel) {
				common.SysLog(fmt.Sprintf("skip auto test for image/video-gen channel #%d (%s, model=%s) — too expensive", channel.Id, channel.Name, autoTestModel))
				continue
			}
