...
func getPayMoney(amount int64, group string) float64 {
	...
	// 充值汇率优先用实时 USDExchangeRate（中国银行外汇牌价 cron 拉取），
	// 与日志详情/余额显示使用同一汇率源，保证客户端到端一致；
	// 仅当 USDExchangeRate 未配置(<=0) 时回退到旧的 Price 字段。
	topupRate := operation_setting.USDExchangeRate
	if topupRate <= 0 {
		topupRate = operation_setting.Price
	}
	dPrice := decimal.NewFromFloat(topupRate)
...
func RequestEpay(c *gin.Context) {
	...
	returnUrl, _ := url.Parse(system_setting.ServerAddress + "/topup-receipt?trade_no=" + tradeNo)
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			model.FireTopupSuccess(int64(topUp.Id), topUp.UserId, topUp.Money)
		}
	} else {
...
// GetAllTopUps 管理员获取全平台充值记录。?user_id=N 可按用户过滤。
...
// GetSelfTopUpByTradeNo 用户查询自己某笔订单（按 trade_no）
// GET /api/user/topup/self/trade/:trade_no
func GetSelfTopUpByTradeNo(c *gin.Context) {
	userId := c.GetInt("id")
	if userId == 0 {
	...
	if topUp == nil || topUp.UserId != userId {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "订单不存在或不属于您"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"success": true, "data": topUp})
}

// DeleteUserTopUp 用户删除自己的 pending/failed/expired 订单
// DELETE /api/user/topup/self/:id
func DeleteUserTopUp(c *gin.Context) {
	userId := c.GetInt("id")
	if userId == 0 {
		c.JSON(http.StatusUnauthorized, gin.H{"success": false, "message": "未登录"})
		return
	...
	if err := model.DeleteUserPendingOrFailedTopUp(userId, id); err != nil {
		switch err {
		case model.ErrTopUpNotFound:
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "订单不存在或不属于您"})
		case model.ErrTopUpStatusInvalid:
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "已成功的订单不允许删除"})
		default:
			common.ApiError(c, err)
		}
		return
