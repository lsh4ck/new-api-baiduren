...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		// Webhook secret is mandatory. Skipping verification in "test mode"
		// would allow unauthenticated callers to trigger arbitrary top-ups.
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret 未配置, 拒绝请求 signature=%q body=%q", signature, payload))
		return false
	}
